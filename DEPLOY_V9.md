...
async function matchImageFromLibrary(db, params) {
  ...
  try {
    ...
    if (best.score >= MIN_MATCH_SCORE) {
      ...
      console.log(`[图片库] 匹配成功: 分数=${best.score}`)
      return { image: best.image, score: best.score, needAI: false }
    } else {
      console.log(`[图片库] 匹配度${best.score}不足`)
...
function generateImagePrompt(page, styleConfig) {
  ...
  try {
    const response = await fetch(ARK_CHAT_API, {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: `为"${topic}"的"${section}"章节生成${count}个PPT要点，每行一个，不要序号，简洁有力。`
        }]
      })
    })
  ...
  try {
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      if (!match.needAI && match.image) {
        ...
        const prompt = generateImagePrompt(page, styleConfig)
...
### 2. 创建 `image_library` 集合
- 集合名称：`image_library`
- 点击确定

### 3. 导入初始数据
点击"插入文档"，逐条添加以下数据（或使用批量导入）：

```json
[
  {
    "_id": "tpl_guobao_001_cover",
    "path": "templates-assets/images/tpl_guobao_001/image1.png",
    "type": "cover",
    "style": "party_red",
  ...
  },
  {
    ...
    "path": "templates-assets/images/tpl_guobao_001/image2.png",
    "type": "content",
    "style": "party_red",
    "keywords": ["国宝", "文化", "展示"],
...
**提示**：完整数据在 `templates-assets/image-library-index.json` 文件中
...
### 1. 测试API
```bash
curl -X POST https://ig8u65l6vm.sealosbja.site/generate \
  -H "Content-Type: application/json" \
  -d '{"topic":"AI技术发展报告","pageCount":5}'
```

### 2. 检查日志
- 在Sealos控制台查看函数日志
- 应该看到 `[图片库] 匹配成功` 或 `[AI生图] 成功` 的日志

---

## 四、注意事项

1. **环境变量**：确保 `ARK_API_KEY` 已设置
2. **图片URL**：图片库中的 `path` 字段需要是可访问的URL
3. **初次使用**：图片库数据较少时，会较多使用AI生图
